...
5. 线程池中线程数量的增减时机:
   ...
   2. 销毁线程的时机:
      ...
      3. <font color=#ff4500> 注意: 当线程池还处于运行中,阻塞队列里面还有任务,并且现有线程仅剩1个的时候,线程是不允许回收的.</font>    
...
7. 真正的打工仔: worker
   ...
   2. 重要参数:
      ...
      在需要中断线程的时候,通过AQS中tryAcquire获取锁的时候,compareAndSetState(0, 1)去获取锁,自然无法成功获取锁,保证了正在运行的线程无法被中断
